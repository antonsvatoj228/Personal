...
def build(wb, data):
    ...
    rows = [
      ...
      (22,'Проживання до знижки, грн','=F11*IF($B$9="Вихідні",F13,1)+F14*КРОК_ГОСТЯ','#,##0.00'),
      (23,'ЦІНА ЗА НІЧ, грн',
       '=IF($F$6<>"OK","",ROUND((F22*(1-F15)+($B$10*F18+$B$11*F19)*(1-F15*F16)'
       '+F20*(1-F15*F17)+F21)/ОКРУГЛЕННЯ,0)*ОКРУГЛЕННЯ)',MONEY),
